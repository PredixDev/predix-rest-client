...
public class CFRestMultipleClientIT
{
    ...
    @Value("${predix.asset.zoneid1}")
    private String              zoneId1;
    
    @Value("${predix.asset.zoneid2}")
...
//    @Test
            ...
            throws IOException
    {
        ...
        CloseableHttpResponse response = this.restClient.get(url, headers,
                this.restConfig1.getDefaultConnectionTimeout(), this.restConfig1.getDefaultSocketTimeout());
            ...
            throws IOException
    {
        ...
        headers.add(new BasicHeader("Predix-Zone-Id", this.zoneId1));
            ...
            throws IOException
    {
        ...
        headers.add(new BasicHeader("Predix-Zone-Id", this.zoneId2));

        CloseableHttpResponse response = this.restClient2.get(url, headers,
                this.restConfig2.getDefaultConnectionTimeout(), this.restConfig2.getDefaultSocketTimeout());
        Assert.assertNotNull(response);
        int httpStatus = response.getStatusLine().getStatusCode();
        Assert.assertEquals(200, httpStatus);
        String responseString = this.restClient.getResponse(response);
    ...
    private void getOneAsset()
    {
        ...
        log.info("Get One Asset zone=" + this.zoneId1);
        log.info("Get One Asset oauthClientId=" + this.restConfig1.getOauthClientId());
        log.info("Get One Asset getOauthClientIdEncode=" + this.restConfig1.getOauthClientIdEncode());

        List<Header> headers = this.restClient.getSecureTokenForClientId();
        headers.add(new BasicHeader("Predix-Zone-Id", this.zoneId1));
        @SuppressWarnings("resource")
        HttpResponse response = this.restClient.get(url, headers, this.restConfig1.getDefaultConnectionTimeout(),
                this.restConfig1.getDefaultSocketTimeout());
        Assert.assertNotNull(response);
        int httpStatus = response.getStatusLine().getStatusCode();
        String responseString = this.restClient.getResponse(response);
        log.info("url=" + url);
    ...
    private void delete()
    {
        ...
        headers.add(new BasicHeader("Predix-Zone-Id", this.zoneId1));
        @SuppressWarnings("resource")
        HttpResponse response = this.restClient.delete(url, headers, this.restConfig1.getDefaultConnectionTimeout(),
                this.restConfig1.getDefaultSocketTimeout());
        int httpStatus = response.getStatusLine().getStatusCode();
        assert (httpStatus == 204);
        Assert.assertNotNull(response);
